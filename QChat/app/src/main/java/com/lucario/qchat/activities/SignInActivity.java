...
import com.lucario.qchat.databinding.ActivitySignInBinding;
...
public class SignInActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(preferenceManager.getString("ggh-public-key") == null){
            new Thread(new Runnable() {
                @Override
                public void run() {
                    GGH ggh = new GGH(10, ThreadLocalRandom.current().nextInt(100, 1000));
                    String privateBasis = ggh.getPrivateBasis();
                    String publicBasis = ggh.getPublicBasis();
                    String badVector = ggh.getBadVector();

                    preferenceManager.putString("ggh-public-key", publicBasis);
                    preferenceManager.putString("ggh-private-key", privateBasis);
                    preferenceManager.putString("ggh-bad-vector", badVector);
                }
            }).start();
        }
